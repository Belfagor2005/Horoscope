...
TMPPATH=/tmp/Horoscope-install
FILEPATH=/tmp/Horoscope-main.tar.gz

echo "Starting Horoscope installation..."
...
cleanup() {
    ...
    [ -d "/tmp/Horoscope-main" ] && rm -rf "/tmp/Horoscope-main"
}
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract Horoscope package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/Horoscope-main/usr/lib/enigma2/python/Plugins/Extensions/horoscope" ]; then
    cp -r "$TMPPATH/Horoscope-main/usr/lib/enigma2/python/Plugins/Extensions/horoscope"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/Horoscope-main/usr/lib64/enigma2/python/Plugins/Extensions/horoscope" ]; then
    cp -r "$TMPPATH/Horoscope-main/usr/lib64/enigma2/python/Plugins/Extensions/horoscope"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/Horoscope-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/Horoscope-main/usr"/* /usr/ 2>/dev/null
